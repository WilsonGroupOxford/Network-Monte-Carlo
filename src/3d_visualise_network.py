...
class Network:
    ...
    def read_dual_file(self):
        ...
        if self.avCnxs>4.5: self.avCnxs=6
        elif self.avCnxs>3.5: self.avCnxs=4
        else: self.avCnxs=3
        f.close()
...
#
        ...
        for i, ring in enumerate(self.ring_crds):
            if np.max(self.ring_crds[i])>lim: lim=np.max(self.ring_crds[i])
            ...
            alpha = (delta)/(2*np.pi)
            alpha=1 
            ax.add_collection3d(Poly3DCollection([self.ring_crds[i]],alpha=alpha,facecolor=self.ring_colours[ring[:, 0].size],edgecolor="k",linewidth=lw))
...
#         ax.add_collection(PatchCollection(patches, facecolor=ring_colours, edgecolor='k', linewidths=lw, alpha=alpha, zorder=zorder))
    ...
    def plot_connections(self,ax,lw=1):
        ...
        for i, cnxs in enumerate(self.netCnxs):
            ...
            for j in cnxs:
                ax.plot(xs=(self.crds[i,0],self.crds[j,0]),ys=(self.crds[i,1],self.crds[j,1]),zs=(self.crds[i,2],self.crds[j,2]),c="k",lw=lw)
