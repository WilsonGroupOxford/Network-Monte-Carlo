    ...
    def generate_ring_clusters(self,r_network):
        ...
        for i,cnxs in enumerate(r_network.netCnxs):
            if len(cnxs)==clst_type:
                n = 0
                for j in cnxs:
                    if len(r_network.netCnxs[j])==clst_type: n += 1
                if n>=min_clst_cnxs:
                    active_clst[i]=1
                    active_aux[i]=0
                elif n>=min_aux_cnxs:
            ...
            if active_clst[i]:
                ...
                while True:
                    for j in search0:
                        ...
                        for k in ids:
                            if active_clst[k]: search1.add(k)
                            elif active_aux[k]: search1a.add(k)
                    if len(search1)==0 and len(search1a)==0: break
                    for j in clst:
                        if j in search1:
                            search1.remove(j)
                        if j in search1a:
    ...
    def plot_connections(self,ax,lw=1,alpha=1,x_shift=0,y_shift=0):
        ...
        for i, cnxs in enumerate(self.netCnxs):
            ...
            for j in cnxs:
                ...
                if x>x_cut: x-=self.pb[0]
                elif x<-x_cut: x+=self.pb[0]
                if y>y_cut: y-=self.pb[1]
                elif y<-y_cut: y+=self.pb[1]
                #if i<j:
                plt.plot((self.crds[i,0],self.crds[i,0]+x),(self.crds[i,1],self.crds[i,1]+y),c="k",lw=lw,alpha=alpha,zorder=1)
                # else:
                #     plt.plot((self.crds[i,0],self.crds[i,0]+x),(self.crds[i,1],self.crds[i,1]+y),c="k",lw=lw/2,alpha=alpha+0.5,zorder=1,ls='--')
        self.crds[:,0]-=x_shift
        self.crds[:,1]-=y_shift
        ax.set_xlim(-self.pb[0]*0.5,self.pb[0]*1.5)
        ax.set_ylim(-self.pb[0]*0.5,self.pb[0]*1.5)
